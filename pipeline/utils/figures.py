...
def plot_roc_curves(models_dict, save_dir=None):
    ...
    None
    """
    save_dir = save_dir + "roc_curve" if save_dir else ""
    check_and_create_directory(save_dir)
        ...
        try:
            ...
            for fold_data in results:
                ...
                for i in range(len(classes)):
                    ...
                    fpr, tpr, _ = roc_curve(y_true, probas[:, i], pos_label=class_i)
            ...
            for class_i in mean_tprs.keys():
                ...
                plt.plot(mean_fpr, mean_tpr, lw=2, color=color_dict[class_i], label=f'Class {class_i} mean ROC (area = {mean_auc:0.2f})')
