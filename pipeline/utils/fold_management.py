...
class fold_management: 
    ...
    def log_combinations(self, log_F, targets, metadata):
        ...
        for i, (train_indices, test_indices) in enumerate(self.generator):
            ...
            log_F.info("############################################")
            log_F.info(f"Fold {i + 1}:")
            log_F.info(f"Number of Train Samples: {len(train_indices)}")
            log_F.info(f"Number of Test Samples: {len(test_indices)}")
            log_F.info(f"Distribution class train: {train_distribution}")
            log_F.info(f"Distribution class test: {test_distribution}")
            log_F.info(f"Train size: {train_size}")
            log_F.info(f"Massives in train: {train_massives}")
            log_F.info(f"Massives in test: {test_massives}")
